...
ARCHIVE="$DERIVED/AIMeter.xcarchive"
EXPORT="$DERIVED/export"
APP="$EXPORT/AI Meter.app"
...
SPARKLE_ACCOUNT="${SPARKLE_ACCOUNT:-AIMeter}"
GITHUB_REPOSITORY="${GITHUB_REPOSITORY:-JewhurstEngineering/ai-meter}"
...
if [[ "$CONFIGURATION" != "Release" ]]; then
  xcodebuild \
    ...
    build
  print "Built $DERIVED/Build/Products/$CONFIGURATION/AI Meter.app"
  exit 0
fi
...
xcodebuild \
  ...
  -archivePath "$ARCHIVE" \
  -exportPath "$EXPORT" \
  -exportOptionsPlist "$ROOT/Scripts/export-options-mac-developer-id.plist" \
  -allowProvisioningUpdates

[[ -d "$APP" ]] || fail "Export succeeded but $APP was not produced."
...
codesign --force --sign "$DEVELOPER_ID" --options runtime --timestamp \
  "$SPARKLE_FRAMEWORK"
codesign --force --sign "$DEVELOPER_ID" --options runtime --timestamp \
  --entitlements "$ROOT/Apps/macOS/AIMeter/AIMeter.entitlements" \
  "$APP"
...
mkdir -p "$ARTIFACTS"
STAGING="$(mktemp -d "$DERIVED/AIMeter-release.XXXXXX")"
trap 'rm -rf "$STAGING"' EXIT

NOTARY_ZIP="$STAGING/AIMeter-notary.zip"
ditto -c -k --sequesterRsrc --keepParent "$APP" "$NOTARY_ZIP"

print "Submitting AI Meter $VERSION ($BUILD_NUMBER) for notarization…"
...
ARCHIVE_NAME="AIMeter-$VERSION.zip"
