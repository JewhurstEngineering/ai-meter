...
/// Saved provider session (metadata only). Token lives in Keychain under `id.uuidString`.
...
public enum AccountRegistryStore {
    ...
    public static func migrateLegacyPrimaryIfNeeded(
        ...
        let id = UUID()
        registry.connections = [AccountConnection(id: id, email: nil, label: "", createdAt: Date(), provider: .cursor)]
    ...
    public static func planLegacyMigration(
        ...
        return AccountRegistry(
            connections: [AccountConnection(id: id, email: nil, label: "", createdAt: Date(timeIntervalSince1970: 0), provider: .cursor)],
            activeAccountID: id
        )
    }
}
